...
RegisterNetEvent('Friks-sellped:giveinput', function (data)
    local header = 'Item: '..data.item
    local input = lib.inputDialog(header, {
        { type = 'input', label = 'Müügisumma', placeholder = '10' },
        { type = 'select', label = 'Makseviis', options = {
            { value = 'cash', label = 'sularaha', icon = 'fas fa-wallet'},
    ...
    if input[1] ~= nil then
        if input[2] ~= nil then
            TriggerServerEvent('Friks-sellped:sellitem',data.item, data.price, input[1], input[2])
        else
            QBCore.Functions.Notify('Makseviisi pole valitud.', 'error', 4500)
        end
    else
        QBCore.Functions.Notify('Summat ei antud.', 'error', 4500)
    end
end)